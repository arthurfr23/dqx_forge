...
export function generateDatabricksYml(nome: string, host: string): string {
  ...
  return `# Created by DQX Forge because the project had no bundle yet.
# Unlike resources/, this file is yours: edit it freely, it is never
# overwritten by later generations.
...
export function applyDashboardParameters(
  ...
  const alvo = contracts.find((c) => c.output.metrics_table) ?? contracts[0];
  if (!alvo) {
    throw new Error(t().dash_semContrato);
  }

  const curada = partes(alvo.output.output_table ?? alvo.meta.table);
  ...
  const invalida =
    alvo.output.mode === "quarantine" && alvo.output.quarantine_table
      ? partes(alvo.output.quarantine_table)
      : curada;
  const metricas = alvo.output.metrics_table ? partes(alvo.output.metrics_table) : undefined;
...
function withHeader(body: string): string {
  return `# Generated by DQX Forge from the contracts in dq/contracts/.
# Do not edit by hand: run "DQX Forge: Generate bundle resources" again.
