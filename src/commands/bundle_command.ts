 ...
 */
export async function generateBundleResources(store: ContractStore): Promise<void> {
  ...
  const contractsDir = config.get<string>("contractsDir", "sat_bh/dq/contracts");

  // resources/ fica ao lado do databricks.yml, um nível acima de dq/contracts.
  const bundleRoot = contractsDir.split("/").slice(0, -2).join("/");
  const resourcesDir = vscode.Uri.joinPath(
    raiz.uri,
    ...[...bundleRoot.split("/").filter(Boolean), "resources"],
  );

  const options = {
    dqxVersion: config.get<string>("dqxVersion", "0.15.0"),
    contractsDir: contractsDir.split("/").slice(1).join("/"),
  ...
  if (options.warehouseId) {
    ...
    const dashboardFile = vscode.Uri.joinPath(
      raiz.uri,
      ...[...bundleRoot.split("/").filter(Boolean), "dq", "dashboard", "dqx_quality.lvdash.json"],
    );
    try {
